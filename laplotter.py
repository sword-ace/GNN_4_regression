...
class LossAccPlotter(object):
    ...
    def add_values(self, x_index, loss_train=None, loss_val=None, acc_train=None,
        ...
        acc_train = ignore_nan_and_inf(acc_train, "error train", x_index)
        acc_val = ignore_nan_and_inf(acc_val, "error val", x_index)
    ...
    def redraw(self):
        ...
        for ax, label in zip([ax1, ax2], ["Loss", "Error"]):
            if ax:
                ax.clear()
                ax.set_title(label)
                ax.set_ylabel(label)
        ...
        if ax1:
            ax1.legend([label.replace("$CHART", "loss") for label in labels],
                       ...
                       ncol=ncol)
        if ax2:
            ax2.legend([label.replace("$CHART", "error") for label in labels],
                       loc="upper center",
                       bbox_to_anchor=(0.5, -0.08),
                       ncol=ncol)
    ...
    def _redraw_main_lines(self):
        ...
        if ax2:
            h_at, = ax2.plot(list(self.values_acc_train.keys()), list(self.values_acc_train.values()),
                             ls_acc_train, label="error train", alpha=alpha_main)
            h_av, = ax2.plot(list(self.values_acc_val.keys()), list(self.values_acc_val.values()),
                             ls_acc_val, label="error discriminator", alpha=alpha_main)
